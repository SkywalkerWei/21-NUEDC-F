...
E0=0
Find_task=0
def send_data(outn,lr,task):
    global uart
    data=struct.pack("<bbbbbbb",0x2C,0x12,outn,lr,0x01,task,0x5B)
    uart.write(data)
clock = time.clock()
while (True):
        clock.tick()
        ...
        while(A0):
            ...
            if r2:
                ...
                A0=0
            roi0=(60,50,40,40)
            for obj0 in tf.classify(net, img, roi=roi0, min_scale=1, scale_mul=0.8, x_overlap=0.5, y_overlap=0.5):
                print("**********\nPredictions at [x=%d,y=%d,w=%d,h=%d]" % obj.rect())
                img.draw_rectangle(obj0.rect())
                predictions_list0 = list(zip(labels, obj0.output()))
                max0=predictions_list0[1][1]
                num=3
            for i0 in range(len(predictions_list0)):
                print("%s = %f" % (predictions_list0[i0][0], predictions_list0[i0][1]))
                if predictions_list0[i0][1]>max0 and (ord(predictions_list0[i0][0])-48) != 0:
                    max0=predictions_list0[i0][1]
                    num=ord(predictions_list0[i0][0])-48
                    E0=1
            A0=0
        while(A9):
            if A1==1:
