...
sensor.set_windowing(0, 35, 160, 60)
#sensor.set_windowing(60, 0, 40, 40)
# 如何确定这个窗口的范围：注释掉，框选出数字占据的部分
# 在这个状态下，五个位置的图全部拍到
...
def FirstFindTemplate(template):
    R = img.find_template(template, 0.85, step=1, roi=(0, 0, 160, 60), search=SEARCH_EX)
    return R
...
def FindTemplate(template):
    R = img.find_template(template, 0.8, step=1, roi=(0, 0, 160, 60), search=SEARCH_EX)
    return R
...
def FindedNum(R, Finded_Num):
   ...
   #本来中值是80的，但返回值是框边缘，所以减去20  小于65是在左边，大于65是在右边
   if R[0] >= 60:
       LoR = 2    #2是右
   elif 0< R[0] <60:
       LoR = 1     #1是左
   find_flag = 1
   Num = Finded_Num
   FH = bytearray([0x2C,0x12,Num, LoR, find_flag, Find_Task,0x5B])#看到的数字，左右，，确认目标还是在去目标
   uart.write(FH)
