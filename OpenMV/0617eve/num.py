...
def outuart(x,num):
    ...
    data = ustruct.pack("<bbhhhhb",      #格式为俩个字符俩个短整型(2字节)
                   0x2C,                      #帧头1
                   0x12,                      #帧头2
                   int(num), # up sample by 4   #数据1
                   int(lr), # up sample by 4    #数据2
                   int(0), # up sample by 4    #数据1
                   int(2), # up sample by 4    #数据2
                   0x5B)
    for x in range(5):
...
def send_data(outn,lr,flag,task):
    global uart
    data=ustruct.pack("<bbbbbbb",0x2C,0x12,outn,lr,flag,task,0x5B)
    uart.write(data)
