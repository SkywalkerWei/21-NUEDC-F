...
template3 = ["/3.pgm","/3a.pgm","/3b.pgm","/3c.pgm","/3d.pgm","/3e.pgm"]
        ...
        while(True):
            while(C0):
                ...
                for i in range(len(predictions_list)):
                    ...
                    if predictions_list[i][1]>0.70 and num1==num:
                        red_led.on()
                        time.sleep_ms(500)
                        red_led.off()
                        if sended==0:
                        ...
                        C1=1
                roiL2=(45,20,35,50)
                for obj in tf.classify(net, img, roi=roiL2,min_scale=1, scale_mul=0.8, x_overlap=0.5, y_overlap=0.5):
                    print("**********\nPredictions at [x=%d,y=%d,w=%d,h=%d]" % obj.rect())
                    img.draw_rectangle(obj.rect())
                    predictions_list3 = list(zip(labels, obj.output()))
                for ii in range(len(predictions_list3)):
                    print("%s = %f" % (predictions_list3[ii][0], predictions_list3[ii][1]))
                    num11=ord(predictions_list3[ii][0])-48
                    if predictions_list3[ii][1]>0.70 and num11==num:
                        red_led.on()
                        time.sleep_ms(500)
                        red_led.off()
                        if sended==0:
                ...
                for p in range(len(predictions_list2)):
                    ...
                    if predictions_list2[p][1]>0.70 and num2 == num:
                        blue_led.on()
                        time.sleep_ms(500)
                        blue_led.off()
                        if sended==0:
                        ...
                        C1=1
                roiR2=(115,30,35,35)
                for obj in tf.classify(net, img, roi=roiR2,min_scale=1, scale_mul=0.8, x_overlap=0.5, y_overlap=0.5):
                    print("**********\nPredictions2 at [x=%d,y=%d,w=%d,h=%d]" % obj.rect())
                    img.draw_rectangle(obj.rect())
                    predictions_list4 = list(zip(labels, obj.output()))
                for pp in range(len(predictions_list4)):
                    print("%s = %f" % (predictions_list4[pp][0], predictions_list4[pp][1]))
                    num4=ord(predictions_list2[pp][0])-48
                    if predictions_list2[pp][1]>0.70 and num4 == num:
                        blue_led.on()
                        time.sleep_ms(500)
                        blue_led.off()
                        if sended==0:
